...
func ProcessIndexEvent(ctx context.Context, event sqlc.Event, q *sqlc.Queries) error {
	...
	case "product.image.added":
		return processProductImageAdded(ctx, event.EventPayload, q)

	case "product.image.removed":
		return processProductImageRemoved(ctx, event.EventPayload, q)

	// case "product.image.set_primary":
	// 	return processProductPrimaryImageSet(ctx, event.EventPayload, q)

	case "product.promo_video.added":
		return processProductPromoVideoAdded(ctx, event.EventPayload, q)
